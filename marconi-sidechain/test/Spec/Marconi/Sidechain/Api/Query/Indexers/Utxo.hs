...
module Spec.Marconi.Sidechain.Api.Query.Indexers.Utxo
  ( tests
  , rpcTests
  ) where
...
initRpcTestEnv = do
  ...
  let f= \s -> Warp.testWithApplication (pure $ marconiApp ixenv)(rpcHttpClient s)
  pure (mkRpcEnv ixenv, f)

rpcTests  :: TestTree
rpcTests = withResource initRpcTestEnv (\_ -> pure ()) runRpcTests

runRpcTests :: RpcClientAction -> TestTree
runRpcTests rpcenv = testGroup "marconi-sidechain -utxo json-rpc test-group"
    [ testPropertyNamed
...
rpcHttpClient
  ...
  -> (Warp.Port -> IO (JsonRpcResponse String UtxoQueryResult))
rpcHttpClient address port = do
  manager' <- newManager defaultManagerSettings
  let env = mkClientEnv manager' (baseUrl port)
  let (rpcEcho :<|> rpcTargets :<|> rpcUtxos) = mkHoistedHttpRpcClient env
  -- test will fail if these calls/routes fail.
...
propUtxoEventInsertionAndJsonRpcQueryRoundTrip action = property $ do
  ...
  let (inserted :: Set Utxo.UtxoRow) = Set.fromList . concatMap Utxo.eventsToRows $ events
      (fetched :: Set Utxo.UtxoRow) =  Set.fromList . concatMap fromQueryResult $ rpcResponses
  -- TODO
  -- May be remove in favor of the `inserted === fetched` test, when the serialisation bug is fixed
  when (any rpcQueryHasFailed rpcResponses ) Hedgehog.failure -- TODO remove me when we solve the test failer re ScriptData serialization difference^
  length fetched  === length inserted -- TODO remove when the provious `todo` is fixed
  -- TODO --
  -- uncomment once the ScriptData serialisation is fixed
  -- fetched === inserted   -- This is the only test required.
...
-- | Is there a failure in the JSON-RPC response
rpcQueryHasFailed :: JsonRpcResponse e UtxoQueryResult -> Bool
rpcQueryHasFailed (Result _ _ ) = False
rpcQueryHasFailed _             = True
